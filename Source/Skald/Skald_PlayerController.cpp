...
void SpawnImpactEffect(UWorld *World, UFXSystemAsset *Effect,
  ...
  if (UNiagaraSystem *NiagaraEffect = Cast<UNiagaraSystem>(Effect)) {
    UNiagaraFunctionLibrary::SpawnSystemAtLocation(World, NiagaraEffect, Location,
                                                   Rotation);
  } else if (UParticleSystem *CascadeEffect = Cast<UParticleSystem>(Effect)) {
    UGameplayStatics::SpawnEmitterAtLocation(World, CascadeEffect, Location,
                                             Rotation);
  }
}
}
...
void ASkaldPlayerController::PlayAttackFeedback(
  ...
  if (bAnyDamage) {
    SpawnImpactEffect(World, HitImpactEffect, ImpactLocation, ImpactRotation);
    if (HitImpactSound) {
      UGameplayStatics::PlaySoundAtLocation(this, HitImpactSound,
                                            ImpactLocation);
    }
  } else {
    SpawnImpactEffect(World, MissImpactEffect, ImpactLocation, ImpactRotation);
    if (MissImpactSound) {
