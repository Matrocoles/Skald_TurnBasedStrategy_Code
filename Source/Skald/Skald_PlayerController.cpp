...
void ASkaldPlayerController::ServerSelectTerritory_Implementation(
  ...
  if (Territory && bOwnershipRequired && Territory->OwningPlayer != PS) {
    const UEnum *TurnPhaseEnum = StaticEnum<ETurnPhase>();
    const FString PhaseName = TurnPhaseEnum
                                  ? TurnPhaseEnum->GetNameStringByValue(
...
void ASkaldPlayerController::HandleTerritorySelected(ATerritory *Terr) {
  ...
  if (!Terr->IsSelectionVisibleToLocalPlayer()) {
    return;
  }

  const FString OwnerName = ResolvePlayerName(Terr->OwningPlayer,
...
bool ASkaldPlayerController::ShouldDisplayPrepareForBattlePrompt(
  ...
  if (bMatchesAttacker) {
    ...
    const bool bReadyStateConflictsWithPrompt =
        bReadyStateHasAttackerId &&
        ReadyState.AttackerPlayerID != PromptData.AttackerPlayerID;

    if (bReadyStateConflictsWithPrompt) {
      UE_LOG(LogSkaldReady, Warning,
             TEXT("Proceeding with prepare prompt for %s despite attacker ready state mismatch (ReadyState=%d Prompt=%d)."),
             *GetName(), ReadyState.AttackerPlayerID, PromptData.AttackerPlayerID);
    } else if (!bReadyStateHasAttackerId) {
      UE_LOG(LogSkaldReady, Verbose,
  ...
  if (bMatchesDefender) {
    ...
    const bool bReadyStateConflictsWithPrompt =
        bReadyStateHasDefenderId &&
        ReadyState.DefenderPlayerID != PromptData.DefenderPlayerID;

    if (bReadyStateConflictsWithPrompt) {
      UE_LOG(LogSkaldReady, Warning,
             TEXT("Proceeding with prepare prompt for %s despite defender ready state mismatch (ReadyState=%d Prompt=%d)."),
             *GetName(), ReadyState.DefenderPlayerID, PromptData.DefenderPlayerID);
    } else if (!bReadyStateHasDefenderId) {
      UE_LOG(LogSkaldReady, Verbose,
...
void ASkaldPlayerController::ShowPrepareForBattlePromptLocal(
  ...
  if (bShouldDeferLocalAuthorityPrompt) {
    if (UWorld *World = GetWorld()) {
      ...
      const uint64 ScheduledSequence = ++DeferredPreparePromptSequenceCounter;
      ActiveDeferredPreparePromptSequence = ScheduledSequence;

      FTimerDelegate DeferredDelegate = FTimerDelegate::CreateWeakLambda(
          this, [this, ScheduledSequence]() {
            ExecuteDeferredPrepareForBattlePrompt(ScheduledSequence);
          });
      World->GetTimerManager().SetTimerForNextTick(DeferredDelegate);
      return;
    }
  }
...
void ASkaldPlayerController::CancelDeferredPrepareForBattlePrompt() {
  ...
  ActiveDeferredPreparePromptSequence = 0;
