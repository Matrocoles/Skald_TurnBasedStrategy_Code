...
void UW_DiceResolutionPanel::OverrideDiceFaceTextures(const TArray<TObjectPtr<UTexture2D>>& InTextures)
{
...
void UW_DiceResolutionPanel::RevealNextDie()
{
    ...
    if (OutcomeList)
    {
        if (UWidget* Entry = BuildOutcomeWidget(Outcome, RevealIndex))
        {
...
UWidget* UW_DiceResolutionPanel::BuildOutcomeWidget(const FDiceRollOutcome& Outcome, int32 DisplayIndex)
{
    UHorizontalBox* Row = NewObject<UHorizontalBox>(OutcomeList);
    if (!Row)
    {
        return nullptr;
    }

    const FLinearColor Tint = ResolveOutcomeTint(Outcome);

    if (UTexture2D* FaceTexture = ResolveFaceTexture(Outcome.RollValue))
    {
        if (UImage* DieImage = NewObject<UImage>(Row))
        {
            DieImage->SetBrushFromTexture(FaceTexture, true);
            DieImage->SetColorAndOpacity(Tint);

            if (UHorizontalBoxSlot* ImageSlot = Row->AddChildToHorizontalBox(DieImage))
            {
    ...
    if (UTextBlock* EntryText = NewObject<UTextBlock>(Row))
    {
        ...
        EntryText->SetText(EntryLabel);
        EntryText->SetColorAndOpacity(FSlateColor(Tint));

        if (UHorizontalBoxSlot* TextSlot = Row->AddChildToHorizontalBox(EntryText))
        {
...
FLinearColor UW_DiceResolutionPanel::ResolveOutcomeTint(const FDiceRollOutcome& Outcome) const
{
    if (!Outcome.bHit)
    {
        return MissDieTint;
    ...
    return Outcome.bCritical ? CriticalDieTint : HitDieTint;
}

UTexture2D* UW_DiceResolutionPanel::ResolveFaceTexture(int32 RollValue) const
{
    ...
    if (RollValue <= 0)
    {
        return DiceFaceTextures[0];
    }

    const int32 Index = FMath::Clamp(RollValue - 1, 0, DiceFaceTextures.Num() - 1);
