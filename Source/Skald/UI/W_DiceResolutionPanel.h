...
public:
    ...
    /** Override the dice face textures used when creating reveal entries. */
    void OverrideDiceFaceTextures(const TArray<TObjectPtr<UTexture2D>>& InTextures);
...
protected:
    ...
    void UpdateTallies();
    UWidget* BuildOutcomeWidget(const FDiceRollOutcome& Outcome, int32 DisplayIndex);
    FLinearColor ResolveOutcomeTint(const FDiceRollOutcome& Outcome) const;
    UTexture2D* ResolveFaceTexture(int32 RollValue) const;
...
private:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Skald|Battle|Dice", meta=(AllowPrivateAccess="true"))
    TArray<TObjectPtr<UTexture2D>> DiceFaceTextures;

    /** Tint applied to hit dice. */
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Skald|Battle|Dice", meta=(AllowPrivateAccess="true"))
    FLinearColor HitDieTint = FLinearColor(0.12f, 0.76f, 0.45f);

    /** Tint applied to miss dice. */
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Skald|Battle|Dice", meta=(AllowPrivateAccess="true"))
    FLinearColor MissDieTint = FLinearColor(0.55f, 0.55f, 0.58f);

    /** Tint applied to critical hit dice. */
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Skald|Battle|Dice", meta=(AllowPrivateAccess="true"))
    FLinearColor CriticalDieTint = FLinearColor(0.98f, 0.78f, 0.15f);
};
